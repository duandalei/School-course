...
private:
  ...
  void recommendPlan() const
  {
    ...
    {
      for (int c = wgt[i]; c <= cap; c++)
        {
          int alt = dp[c - wgt[i]] + val[i];
          if (alt > dp[c])
          {
            dp[c] = alt;   // 更新最大满意度
            choice[c] = i; // 记录选择的商品索引（用于回溯）
          }
      }
    }

    int maxSat = dp[cap]; // 最大满意度（在预算cap下）
    // 统计每个商品购买的数量
    vector<int> countPerIndex(wgt.size(), 0);
    int currentCap = cap; // 当前剩余容量（从最大容量开始回溯）

    // 根据choice数组回溯，统计每个商品的选择次数
    while (currentCap > 0 && choice[currentCap] != -1)
    {
      int i = choice[currentCap]; // 获取选择的商品索引
      countPerIndex[i] += 1;      // 该商品购买数量+1
      currentCap -= wgt[i];       // 减去该商品的价格（容量）
    }
